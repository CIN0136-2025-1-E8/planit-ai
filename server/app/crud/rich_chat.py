...
class CRUDRichChat:
    ...
    def read_history_from_file(self) -> None:
        if os.path.exists(settings.DEBUG_ADVANCED_CHAT_HISTORY_FILE_PATH):
            with open(settings.DEBUG_ADVANCED_CHAT_HISTORY_FILE_PATH, 'rb') as file:
                self.history = pickle.load(file)
        return

    def write_history_to_file(self) -> None:
        with open(settings.DEBUG_ADVANCED_CHAT_HISTORY_FILE_PATH, "wb") as f:
